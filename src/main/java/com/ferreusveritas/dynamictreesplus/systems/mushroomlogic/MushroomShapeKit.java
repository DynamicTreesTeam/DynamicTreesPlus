...
public abstract class MushroomShapeKit extends ConfigurableRegistryEntry<MushroomShapeKit, MushroomShapeConfiguration> implements Configurable {
    ...
    public static final MushroomShapeKit DEFAULT = new MushroomShapeKit(DynamicTreesPlus.location("default")) {
        ...
        protected void registerProperties() {
            this.register(CURVE_POWER, CURVE_HEIGHT_OFFSET, MIN_AGE_CURVE_FACTOR, MAX_AGE_CURVE_FACTOR);
        }
        ...
        public void generateMushroomCap(MushroomShapeConfiguration configuration, MushroomCapContext context) {
            ...
            DynamicCapCenterBlock centerBlock = properties.getDynamicCapCenterBlock().orElse(null);
            ...
            float fac = (((float)age)/(properties.getMaxAge())) * (factorMax - factorMin) + factorMin;
            ...
            for (int i=1; i<=age; i++){
                ...
                if (nextY != y) y+= (int)Math.signum(fac);

                centerBlock.placeRing(context.level(), context.pos().below(y), radius, i);
